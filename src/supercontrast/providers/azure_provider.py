...
class AzureSentimentAnalysis(Provider):
    ...
    def init_from_env(cls) -> "AzureSentimentAnalysis":
        ...
        if not endpoint or not key:
            raise ValueError("AZURE_TEXT_ANALYTICS_ENDPOINT and AZURE_TEXT_ANALYTICS_KEY must be set")

        return cls(endpoint, key)


class AzureTranslation(Provider):
    def __init__(self, key: str, region: str, source_language: str, target_language: str):
        super().__init__()
        self.client = TextTranslationClient(credential=AzureKeyCredential(key), region=region)
    ...
    @classmethod
    def init_from_env(cls, source_language: str, target_language: str) -> "AzureTranslation":
        key = os.environ.get("AZURE_TEXT_ANALYTICS_KEY")
        region = os.environ.get("AZURE_TRANSLATOR_REGION")
        if not key or not region:
            raise ValueError("AZURE_TEXT_ANALYTICS_KEY and AZURE_TRANSLATOR_REGION must be set")
...
class AzureOCR(Provider):
    ...
    def request(self, request: OCRRequest) -> OCRResponse:
        if isinstance(request.image, str):
            ...
            read_response = self.client.read_in_stream(io.BytesIO(request.image), raw=True)
