...
def provider_factory(task: Task, provider: ProviderType, **config):
    ...
    from supercontrast.providers.aws_provider import (
        AWSOCR,
        AWSSentimentAnalysis,
        AWSTranslate,
    )
    from supercontrast.providers.azure_provider import (
        AzureOCR,
        AzureSentimentAnalysis,
        AzureTranslation,
    )
    from supercontrast.providers.gcp_provider import (
        GCPOCR,
    ...
    if task == Task.SENTIMENT_ANALYSIS:
        ...
        logging.info(
            f"Defaulting to {source_language} and {target_language} for translation"
        )
